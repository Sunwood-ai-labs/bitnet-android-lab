...
TRAIN_FILE="${TRAIN_FILE:-$QVAC_ROOT/data/train-tiny.jsonl}"
OUTPUT_ADAPTER="${OUTPUT_ADAPTER:-$QVAC_ROOT/adapters/my-biomed-tiny.gguf}"
CHECKPOINT_DIR="${CHECKPOINT_DIR:-$QVAC_ROOT/checkpoints-tiny}"
...
"$BIN_DIR/llama-finetune-lora" \
  ...
  --checkpoint-save-steps 1 \
  --checkpoint-save-dir "$CHECKPOINT_DIR" \
  -ngl 99 -c 128 -b 16 -ub 16 -fa off \
  --num-epochs 1
