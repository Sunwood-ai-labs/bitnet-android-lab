...
実機 Android + Termux + `adb forward` SSH 経由で、QVAC Fabric BitNet の LoRA 微調整を試した記録を、公開向けに整理したリポジトリです。

## このリポジトリにあるもの

- 2026年3月23日の実機検証をもとにした公開版メモ
- 1台の Xiaomi / Android 15 / Mali-G52 MC2 端末で動いた、patched local `qvac-fabric-llm.cpp` source build の手順断片
- Termux 側で使った補助スクリプト
- ローカル workaround から切り出した最小 patch
- 実行証跡として残した evidence snippet

公式配布物そのものではなく、再現と検証のための lab note を公開向けに整えたものです。

## 確認できた範囲

| 項目 | 状態 | 補足 |
| --- | --- | --- |
| Windows -> `adb forward` -> SSH -> Termux 接続 | 確認済み | 公開版では秘密鍵パスや端末シリアルは除去済み |
...
3. `QVAC_ROOT` 配下に model / adapter / dataset を配置する
